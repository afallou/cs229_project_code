...
\section{Results}
 \subsection*{Initial results}
  The binning involved with our initial results provided too coarse a model. We could not see any features other than 
 \subsection*{Using the normal equations}
 Our feature matrix, \(X\), which contained the features for every training example, gave a matrix $X^TX$ with a very high condition number. This meant the normal equations were unusuable.
 \subsection*{Limitations of our methods}
  % Delete all this when real results are written up.
...
\bibliography{references_final_proj} % Use the example bibliography file sample.bib
